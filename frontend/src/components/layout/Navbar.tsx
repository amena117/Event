...
export function Navbar() {
    ...
    return (
        <motion.header
            className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-300 ${isScrolled ? "bg-white/90 backdrop-blur-md shadow-sm" : "bg-transparent text-white"
                }`}
            initial={{ y: -100 }}
            animate={{ y: 0 }}
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center h-20">
                    <div className="flex-shrink-0">
                        <Link href="/" className={`font-serif text-3xl font-bold tracking-tight ${isScrolled ? 'text-avoda-green-dark' : 'text-white'}`}>
                            Amen
                        </Link>
                    </div>
                    ...
                    <nav className="hidden md:flex space-x-8">
                        {links.map((link) => (
                            <Link
                                key={link.name}
                                href={link.href}
                                className={`text-sm tracking-wide uppercase font-medium hover:text-avoda-gold transition-colors ${isScrolled ? "text-avoda-green-dark" : "text-white/90"
                                    }`}
                            >
                                {link.name}
                            </Link>
                ...
                ${isScrolled
                    ...
                    <div className="md:hidden flex items-center">
                        <button
                            ...
                            className={isScrolled ? "text-avoda-green-dark" : "text-white"}
                        >
